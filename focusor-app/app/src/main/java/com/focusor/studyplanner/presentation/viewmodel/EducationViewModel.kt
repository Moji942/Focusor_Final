...
class EducationViewModel @Inject constructor(
    ...
    private fun loadSubjects() {
        viewModelScope.launch {
            subjectDao.getAllActiveSubjects().collect { subjectEntities ->
    ...
    private fun loadTodayProgress() {
        viewModelScope.launch {
            studySessionDao.getSessionsByDate(LocalDate.now()).collect { sessions ->
    ...
    private suspend fun generateRecommendations(subjects: List<SubjectEntity>) {
        ...
        subjects.forEach { subject ->
            val recentSessions = studySessionDao.getSessionsBySubject(subject.id)
                .first()
                .take(7)
    ...
    suspend fun addStudySession(input: SessionInput) {
        ...
        val subject = subjectDao.getSubjectById(input.subjectId)
        subject?.let {
            val newCompletedPages = it.completedPages + input.pagesRead
            subjectDao.updateProgress(input.subjectId, newCompletedPages)
            ...
            val recentSessions = studySessionDao.getSessionsBySubject(input.subjectId)
                .first()
            
            val avgSpeed = calculationEngine.calculateWeightedAverageSpeed(recentSessions)
            
            subjectDao.updateReadingSpeed(input.subjectId, speed, avgSpeed)
        }
    }
    ...
    suspend fun updateProgress(subjectId: Long, additionalPages: Int) {
        ...
        subject?.let {
            ...
            subjectDao.updateProgress(subjectId, newCompletedPages)
        }
    }
